...
export default function AccountantDashboardPage() {
  ...
  const load = useCallback(async (quiet = false) => {
    ...
    try {
      ...
      toast.error(
        getSafeErrorMessage(error, "Failed to load accountant dashboard."),
      );
    } finally {
      setLoading(false);
    }
  ...
  const recentAssignments = filtered.slice(0, 8);
    ...
    <div>
      <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-gray-100 bg-white p-6 sm:flex-row sm:items-center">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center justify-center gap-1.5 rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-500 transition-colors hover:text-navy disabled:opacity-50"
        >
          <RefreshCw
            size={13}
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-3">
        <QuickAction
          ...
          title="Assigned Work"
          description="Open all service requests assigned to you."
          tone="teal"
        />
        ...
        <QuickAction
          ...
          title="My Documents"
          description="View documents you sent and received."
          tone="navy"
        />
        ...
        <QuickAction
          ...
          title="Messages"
          description="Chat about requests and follow-ups."
          tone="teal"
        />
      </div>
      ...
      <div className="overflow-hidden rounded-xl border border-gray-100 bg-white">
        <div className="flex flex-col gap-3 border-b border-gray-100 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="font-bold text-navy">Recent Assigned Work</h2>
            <p className="mt-1 text-xs text-gray-400">
              Latest requests assigned to your accountant account.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              href="/accountant/assigned-work"
              className="shrink-0 text-xs font-semibold text-teal hover:text-navy"
            >
              View all
            </Link>
          </div>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        tone === "teal"
          ? "border-teal/20 hover:bg-teal hover:text-white"
          : "border-navy/10 hover:bg-navy hover:text-white",
      )}
    >
      <div
        className={cn(
          ...
          tone === "teal"
            ? "bg-teal/10 text-teal group-hover:bg-white/15 group-hover:text-white"
            : "bg-navy/5 text-navy group-hover:bg-white/15 group-hover:text-white",
        )}
      >
        {icon}
      </div>

      <h3 className="text-sm font-bold text-navy group-hover:text-white">
        {title}
      </h3>

      <p className="mt-1 text-xs leading-relaxed text-gray-400 group-hover:text-white/80">
        {description}
      </p>
    </Link>
  );
